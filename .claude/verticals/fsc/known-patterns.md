...
- **Every 4 weeks:** Refresh with new case data from CSV
- **After each Salesforce release (3x/year):** Check for behavioral changes
- **When Known Issues published:** Add to Active Known Issues section
- **When Slack surfaces new pattern:** Add to Tribal Knowledge
- **When new Gack patterns emerge:** Update Section J with new stack trace IDs and resolutions

**Data Sources:**
- Cases: https://docs.google.com/spreadsheets/d/1DDV68Zie0-QpxMGpwu7ckAOhyXCjURjEybuIz5N2pYA/edit
- Investigations: https://docs.google.com/spreadsheets/d/1oGAOz9ppNh5OYehGLah7HGPiRjDb7jRuxV1rmb4OxeU/edit
- Slack: #tech-prod-help-financial-services-cloud, #help-sell-financial-services-cloud
...
### Test 1
**Input:** "Customer getting 'Couldn't create successor tasks for task' — Action Plan stuck, P1"
**Expected:** → Pattern 1. Check template corruption, workflow interference, SOQL limits. Use CodeSearch to trace error. Check GUS for known bug.

### Test 2
**Input:** "'DocumentChecklistItem an invalid action plan template entity' error when creating Action Plan"
**Expected:** → Pattern 2. Check Sales vs FSC type. If Sales → doc checklist not supported. If FSC → check CRUD permissions.

### Test 3
**Input:** "Action Plans permission set is not visible to assign to users"
**Expected:** → Pattern 3. Check PSL in Company Information. Follow flowchart. Check sandbox vs production.

### Test 4
**Input:** "Tasks in template keep reordering randomly after publishing"
**Expected:** → Pattern 4. Query DisplayOrder values. Fix to sequential integers without gaps.

### Test 5
**Input:** "Guest user submits form but Action Plan never gets created — no error visible"
**Expected:** → Pattern 7. Check flow running user context. Guest user lacks ActionPlan CRUD. Switch to System Context or Platform Event pattern.

### Test 6 (Columbo/Gack)
**Input:** "We are getting this error while creating a new action plan: 1770957752-49415 (-1250433517)"
**Expected:** → Recognize ErrorId format. Look up via Columbo `fetch_gack_details`. If expired, `search_gacks` with "ActionPlan". Match to Gack Pattern A (SQL Exception on `populateDerivedStatusField`). Provide resolution: check template integrity, look for custom triggers, clone template if corrupted. If unresolvable, escalate with stack trace ID.

### Test 7 (Columbo/Gack - Template Publish)
**Input:** "Getting a server error when trying to publish an Action Plan template. ErrorId: 1887234521-98234"
**Expected:** → Recognize ErrorId format. Look up via Columbo. Match to Gack Pattern B (NullPointerException on `HighlightsPanelController.publishTemplate`). Provide workaround: clear cache, try different browser, publish via API. If persists, file GUS bug with stack trace reference.

### Test 8 (Referral)
**Input:** "Customer created a Referral but it doesn't show on the client's record page"
**Expected:** → Section K, Pattern K1. Check if Referrals component is on page layout. Check OWD for Referral object. Verify user has Read permission. Query FinServ__Referral__c to confirm record exists and ReferredById/ReferredToId are correct.

### Test 9 (RSR)
**Input:** "Our Rollup Summary Rule for Total Assets isn't updating — it shows the old value from last month"
**Expected:** → Section L, Pattern L1. Check RSR is Active in Setup → Rollup Summary Rules. Check filter criteria. Run manual Recalculate. Explain async recalculation behavior — not real-time.

### Test 10 (Financial Account)
**Input:** "Financial Account is not showing on the client's record even though it exists in the system"
**Expected:** → Section M, Pattern M1. Check Financial Accounts related list is on page layout. Check user Read permission on FinancialAccount. Check OWD. Query FinancialAccount.PrimaryOwner to confirm linked to correct Contact.

### Test 11 (Timeline)
**Input:** "The FSC Timeline component shows Tasks and Events but not Interaction Summaries"
**Expected:** → Section N, Pattern N2. Guide user to Setup → Timeline → Edit configuration → Add InteractionSummary object with ActivityDate as the date field and Subject as title field. Verify user has Read on InteractionSummary.

### Test 12 (Household)
**Input:** "We set up a Household account but the Financial Accounts of members aren't rolling up to it"
**Expected:** → Section O, Pattern O1. Verify Household account RecordType = Household. Check AccountContactRelation records link members to Household. Check RSRs are active for Household rollup (cross-reference Section L).