...
### Test 8 (Tasks Stuck in Waiting — Inactive User)
**Input:** "Action plan tasks not transitioning correctly. After completing the first task, the next task is showing 'Waiting' status instead of 'In Progress'. Issue started in Oct 2025 and is impacting most of the users."
**Expected:** → DO NOT assume it's a release regression just because it started in Oct 2025. → Ask: "Were any users deactivated around that time?" → Proactively search GUS for W-22451209 and related investigations. → Route to Pattern 9 (Inactive User blocking dependency engine). → Provide the diagnostic query to find ActionPlanItems with inactive owners. → Resolution: Reassign ownership of stuck AP Items to an active user. → DO NOT say "refer to Org CS" — investigate it directly.

### Test 9 (Proactive GUS Search)
**Input:** "Customer's action plan successor tasks are not being created. P2 case."
**Expected:** → BEFORE providing generic diagnosis, search GUS for matching investigations: query for "FSC Action Plans" bugs with keywords "successor", "stuck", "transition". → Reference any matching GUS Work IDs (e.g., W-22451209, W-21482069) in the response. → Then proceed to Pattern 1 or Pattern 9 based on specific symptoms.