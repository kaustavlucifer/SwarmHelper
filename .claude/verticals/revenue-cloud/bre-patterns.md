...
FROM ADM_Work__c 
WHERE Product_Tag__r.Name = 'Revenue Cloud (Core)-Business Rules Engine' 
AND Type__c = 'Bug' 
...
- When Slack surfaces new patterns in #rlm-office-hours or #support-rev-rlm-global-swarm-help

---

## Test Cases

1. **Decision Table stuck in Activation In Progress** → Check Splunk for error; try enable/disable Pricing Settings toggle; for Rate Management DTs, enable Rate Management first; if no error, file GUS investigation
2. **Expression Set deployment fails with random IDs** → Deploy ES Definition only (not version); recreate version in target org manually
3. **"Configuration rule refers to different type of sales transaction item"** → Check if Advanced Configurator was previously enabled; reconfigure Rule Library to use Standard SalesTransactionContextDefinition
4. **"No Decision Table exists with name: StandardTax"** → Assign Tax Admin or Tax Configurator PSL directly to user; verify StandardTax CD and DT are Active
5. **Decision Table bulk lookup limit exceeded (200 inputs)** → Verify DT type is MediumVolume with UsageType=DefaultPricing; investigate MaxBulkLookupInputHbpoDt increase via BRE Libra PM
...
✅ Case data: 114 orgcs cases analyzed (re-verified 2026-05-27), 9 core patterns identified + additional sub-patterns from case review
...
❌ GUS active bugs: query unavailable in orgcs context
   → Run GUS query directly: sf data query --target-org gus
✅ Slack: 100+ tribal knowledge items from #rlm-office-hours, #support-rev-rlm-global-swarm-help, #industries-bre-officehours, #help-bre
✅ Help Docs: https://help.salesforce.com/s/articleView?id=ind.business_rules_engine.htm&type=5

GAPS:
- No code references (CodeSearch not connected)
- GUS active P0/P1/P2 bugs not automatically fetched (W-22617103 manually identified from Slack)
- Lower case volume (114) vs. Billing (500) — fewer patterns but high confidence on core issues

RECOMMENDATION:
- Connect CodeSearch for code-level NullPointerException and DT lookup failure traces
- Run the GUS query to populate Section H with current active bugs
- Revisit patterns after next Salesforce release as BRE limits and behavior often change per release (hash key limit changed in 260, Apex SeeAllData fix in 254, etc.)
```