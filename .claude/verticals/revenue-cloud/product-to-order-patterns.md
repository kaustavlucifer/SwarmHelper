...
- **After each Salesforce release (3x/year):** Check for Product/Order behavioral changes
- **When Known Issues published:** Add to Active Known Issues section
- **When new patterns emerge:** Add to Pattern section

**Data Sources:**
- OrgCS case queries (Product to Order cases)
- GUS investigations: Product Tag = "Revenue Cloud - Product to Order"
- Slack: #revenue-cloud-help

---

## Test Cases

### Test 1: Order Product Creation Error
**Input:** "Getting error 'Order Product Group ID: id value of incorrect type' when creating order from quote with bundle products"  
**Expected:** → Pattern 1 or Pattern 10. Check if nested bundles used. Verify ramp schedule not used with nested bundles. Flatten bundle structure. Re-test order creation.

### Test 2: Product Not in Catalog
**Input:** "User cannot find product when adding to quote. Product search returns no results."  
**Expected:** → Pattern 7. Check product IsActive = true. Verify EffectiveDate/ExpirationDate valid. Confirm Price Book Entry exists for quote's price book. Check product visibility rules.

### Test 3: Quote to Order Fails
**Input:** "Create Order button does nothing. Quote status is Accepted."  
**Expected:** → Pattern 4. Verify Quote.Status = 'Accepted'. Check Quote Line Items have required fields. Review Order creation Flow for errors. Verify Account has billing address.

### Test 4: Bundle Components Missing
**Input:** "Added bundle product to quote but components are not showing up"  
**Expected:** → Pattern 2. Query ProductBundleOption records for bundle. Verify component products active. Check TLE has "Expand Bundle" enabled. Test bundle expansion.

### Test 5: Product Price $0.00
**Input:** "Product showing as $0.00 when added to quote"  
**Expected:** → Pattern 6. Check Price Book Entry exists for product and quote's price book. Verify entry IsActive = true. Check UnitPrice field populated. Test with Standard Price Book.

### Test 6: Order Activation Fails
**Input:** "Cannot activate order. Getting validation error."  
**Expected:** → Pattern 9. Check Order.Status = 'Draft'. Verify all required fields populated. Review validation rules on Order object. Check user has permission to edit Order.Status.

---

*Skill created based on OrgCS case analysis and Revenue Cloud Product to Order best practices*

**Version History:**
- v1.0.0 (2026-05-27): Initial version based on Product to Order case patterns and Revenue Cloud knowledge