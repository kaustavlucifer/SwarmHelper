...
- **Every 4 weeks:** Refresh with new OrgCS case data
- **After each Salesforce release (3x/year):** Check for behavioral changes, new known issues
- **When Known Issues published:** Add to Active Known Issues section
- **When tribal knowledge surfaces new pattern:** Add to Pro Tips section
- **When major integration changes (e.g., Session ID deprecation):** Update affected patterns

**Data Sources:**
- OrgCS case queries (last 6 months)
- GUS investigations: Product Tag = "Revenue Cloud", "DocGen", "OmniStudio"
- Slack: #revenue-cloud-help, #omnistudio

---

## Test Cases

### Test 1: DocGen Permission Issue
**Input:** "User getting 'Insufficient privileges' when trying to access Document Builder in Revenue Cloud"  
**Expected:** → Pattern 1. Check permission sets (DocGen Designer, DocGen User, CLM Runtime User). Verify OmniStudio DocGen enabled. Check user license type. Assign missing permission sets → Verify user can access Document Builder.

### Test 2: Contract Status Change Blocked
**Input:** "Cannot change contract status from Active to Expired. Getting 'Context Use Case Mapping' error"  
**Expected:** → Pattern 2. Check Setup → Context Use Case Mapping exists for Contract object. Verify Target Record Type matches. Check ContractDocumentVersion.IsLocked field. Create/fix Context Use Case Mapping → Verify status change succeeds.

### Test 3: Merge Fields Not Populating
**Input:** "Merge fields showing blank in generated contract document. Fields have values on the contract record."  
**Expected:** → Pattern 4. Check merge field syntax ({{Contract.Field__c}}). Verify Field API name correct. Check field-level security on user. Test with System Admin. After Spring '26, try {{SalesTransactionSource.Field}} for quote fields → Verify fields populate.

### Test 4: DocuSign Authentication Error
**Input:** "Getting 'We can't log you in because of an authentication error' when trying to send contract for e-signature"  
**Expected:** → Pattern 9. Check Named Credentials authentication status. Verify DocuSign Connected App configuration (Integration Key, Redirect URI). Re-authenticate Named Credential. Check callback URL matches → Verify envelope creation succeeds.

### Test 5: Document Generation Timeout
**Input:** "Document generation timing out after Spring '26 release. Used to take 10 seconds, now takes 2+ minutes."  
**Expected:** → Pattern 7. Check if images in template (compress to < 500KB). Verify Spring '26 release timing. Enable server-side generation. Check for external storage latency. Test with minimal template → Identify performance bottleneck → Apply optimization.

### Test 6: External Storage Configuration Error
**Input:** "Cannot save contract document to Microsoft 365. Getting 'configuration error or API failure'"  
**Expected:** → Pattern 3. Check Named Credentials configured and authenticated. Verify Contract Type Config has External Storage Configuration set. Test SharePoint API access. Check Azure App Registration permissions → Fix configuration → Verify document saves to M365.

---

*Skill created based on 300 OrgCS cases analyzed (last 6 months) + reference patterns from FSC Action Plans support skill*

**Version History:**
- v1.0.0 (2026-05-26): Initial version based on OrgCS case analysis (Dec 2025 - May 2026)